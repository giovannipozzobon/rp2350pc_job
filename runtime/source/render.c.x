 ...
 *
 * @param[in]  fg    The foreground (if not -ve)
 * @param[in]  bg    The background (if not -ve)
 */
void DVISetMonoColour(int fg, int bg)
{
    if (fg >= 0) palette_mono[0] = fg & 0x7;
    if (bg >= 0) palette_mono[1] = bg & 0x7;
}
